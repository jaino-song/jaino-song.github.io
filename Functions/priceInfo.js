...
function generatePriceMsg(name) {
    ...
    ${name} 산모님~♡ 예약금 관련해서 안내 드립니다 :)
    
    서비스 기간: 출퇴근 ${weeks}주 (평일기준 ${days}일)
    정부지원 바우처 유형: ${type}
    
    기본 서비스 금액은 총 ${fullPrice}원이며, 
    정부 지원금액은 ${grant}원 입니다.
    
    산모님께서 부담하시는 금액은 ${actualPrice}원 입니다.
    
    서비스 예약을 위해 선납하실 예약금은 100,000원 입니다.
    예약금 입금 후에 서비스 예약이 확정 됩니다.
